      ...
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="space-y-3 relative z-10">
            ...
            <div className="flex items-center space-x-3 flex-wrap gap-y-2">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-extrabold transition-all flex items-center space-x-1 border ${
                  copiedAppId 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200/90 shadow-sm space-y-4">
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider border-b pb-3 border-slate-100 flex items-center justify-between">
              ...
              {details.documents && details.documents.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  {details.documents.map((doc, idx) => {
                    ...
                    return (
                      <div key={idx} className={`p-4 bg-slate-50 border ${isRejected ? 'border-rose-300 bg-rose-50/30' : isVerified ? 'border-emerald-200 bg-emerald-50/20' : 'border-slate-200'} rounded-2xl space-y-3`}>
                        ...
                        {isRejected && (
                          <div className="p-3 bg-rose-100/80 border border-rose-200 rounded-xl text-xs text-rose-900 space-y-1">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-slate-200">
                          <div className="flex items-center space-x-2">
                            <a
                              ...
                              className="px-3 py-1.5 bg-[#0b192c] hover:bg-slate-800 text-white font-bold text-xs rounded-xl flex items-center space-x-1"
                            >
                            ...
                            <a
                              ...
                              className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl border border-slate-300 flex items-center space-x-1"
                            >
                          ...
                          {(isRejected || details.status === 'DRAFT') && (
                            <label className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs rounded-xl shadow-sm cursor-pointer flex items-center space-x-1">
                              <span>Replace</span>
                              <input
                                type="file"
                  ...
                  {details.history.map((hist, idx) => (
                    <div key={idx} className="relative">
                      ...
                      <div className="text-xs space-y-1">
                        ...
                        <p className="text-slate-600 bg-slate-50 p-3 rounded-2xl border border-slate-200">{hist.admin_remarks}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200/90 shadow-sm space-y-4">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-2xl border border-slate-200">
                  ...
                  <span className="font-extrabold text-emerald-600 text-sm">₹{details.total_fee || 50}</span>
                </div>
                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-2xl border border-slate-200">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-200/90 shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b pb-3 border-slate-100">
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center space-x-2">
                  ...
                  <span>Officer Support Chat</span>
                </h3>
              </div>
...
function ApplicationChatBox({ applicationId }) {
  ...
  return (
    <div className="space-y-4">
      <div className="max-h-64 overflow-y-auto bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
        {messages.length === 0 ? (
      ...
      <form onSubmit={handleSendMessage} className="flex gap-2">
        <input
          ...
          className="flex-1 bg-slate-50 border border-slate-300 rounded-xl px-4 py-2.5 text-xs text-slate-900 outline-none focus:border-[#0b192c]"
        />
        <button
          type="submit"
          disabled={loading || !newMsg.trim()}
          className="px-4 py-2.5 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-extrabold text-xs rounded-xl shadow transition-colors"
        >
          Send
        </button>
      </form>
