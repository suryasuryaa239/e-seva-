    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-16">
      <div className="bg-gradient-to-r from-slate-900 via-purple-950 to-slate-950 text-white py-12 px-4 sm:px-6 lg:px-8 border-b border-purple-800/40">
        <div className="max-w-7xl mx-auto space-y-4">
          <div className="inline-flex items-center space-x-2 bg-purple-500/20 text-purple-300 border border-purple-400/30 px-3.5 py-1 rounded-full text-xs font-extrabold">
            ...
            <span>{lang === 'ta' ? 'வணிக பதிவு & இணக்க உதவி மையம்' : 'BUSINESS REGISTRATION & COMPLIANCE DESK'}</span>
          </div>

          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
                {lang === 'ta' ? 'வணிகம் & MSME பதிவு சேவைகள்' : 'Business & MSME Compliance Services'}
              </h1>
              <p className="text-slate-300 text-xs sm:text-sm mt-1 max-w-2xl">
                {lang === 'ta' ? 'புதிய ஜிஎஸ்டி (GSTIN), எம்எஸ்எம்இ உத்யம் பதிவு, எப்எஸ்எஸ்ஏஐ (FSSAI) உணவு உரிமம் மற்றும் வர்த்தக உரிமம் ஆன்லைன் விண்ணப்பம்.' : 'Online registration for New GSTIN, MSME Udyam Certificates, FSSAI Food Licenses, and Municipal Trade Licenses.'}
              </p>
            </div>

            <div className="bg-white/10 backdrop-blur-md p-4 rounded-xl border border-white/15 min-w-[240px]">
              <div className="text-[11px] text-purple-200 uppercase font-bold tracking-wider">{lang === 'ta' ? 'வணிக உதவி எண்' : 'Business Helpdesk'}</div>
          ...
          <div className="pt-2 max-w-xl">
            <div className="relative flex items-center">
              ...
              <input
                type="text"
                placeholder={lang === 'ta' ? 'வணிக சேவைகளைத் தேடுங்கள் (GSTIN, MSME உத்யம், FSSAI)...' : 'Search Business services (GSTIN, MSME Udyam, FSSAI, Trade License)...'}
                value={searchTerm}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 space-y-8">
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 flex items-start space-x-3 text-xs text-purple-950 shadow-sm">
          ...
          <div>
            <span className="font-bold text-purple-950">{lang === 'ta' ? 'ஜிஎஸ்டி குறிப்பு:' : 'GST Compliance Note:'}</span> {lang === 'ta' ? 'வருடாந்திர வருவாய் ₹40 லட்சத்தைத் தாண்டும் வணிகங்களுக்கு GST பதிவு கட்டாயமாகும்.' : 'GST registration is mandatory for businesses with turnover exceeding ₹40 Lakhs (Goods) or ₹20 Lakhs (Services), or engaged in inter-state e-commerce.'}
          </div>
        </div>

        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-slate-900">{lang === 'ta' ? `கிடைக்கும் வணிக சேவைகள் (${filtered.length})` : `Available Business Services (${filtered.length})`}</h2>
            <p className="text-xs text-slate-500">{lang === 'ta' ? 'ஆன்லைன் விண்ணப்பத்தைத் தொடங்க சேவையைத் தேர்ந்தெடுக்கவும்.' : 'Select a business service to begin online filing.'}</p>
          </div>
            ...
            return (
              <div key={service.id} className="bg-white rounded-xl border border-slate-200 hover:border-purple-500 hover:shadow-md transition-all flex flex-col justify-between overflow-hidden group">
                ...
                <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                  <Link to={`/enquiry/${service.id}`} className="text-xs font-bold text-slate-700 hover:text-purple-800 hover:underline">
